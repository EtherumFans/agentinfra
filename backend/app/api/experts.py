...
from app.services.a2a_protocol import AgentCard
...
# ---- A2A Protocol + Multi-Agent (extended: retry/cancel) ----

@router.get("/a2a/.well-known/agent.json")
async def a2a_agent_card():
    """A2A Agent Card discovery endpoint (.well-known/agent.json)."""
    from app.config import settings
    return AgentCard(
        name=settings.APP_NAME,
        description="iCoDer Medical Coding Agent — AI-powered ICD-10/ICD-9-CM-3 coding audit",
        url=f"http://localhost:{settings.PORT}/api/experts/a2a",
        version=settings.APP_VERSION,
        capabilities=["diagnosis_coding", "procedure_coding", "fact_extraction", "medical_coding"],
    ).model_dump()


@router.get("/a2a/agents")
async def list_a2a_agents(capability: str = ""):
    """Discover A2A-compatible agents."""
    from app.services.a2a_protocol import a2a_registry
    agents = a2a_registry.discover_agents(capability)
...
async def create_a2a_task(
    ...
    query: str = Query(...),
    capabilities: str = Query("diagnosis_coding"),
):
    """Create an A2A task for multi-agent coordination."""
    from app.services.a2a_protocol import a2a_registry

    # Auto-register iCoDer agents
    for name, caps in [
        ("ICD-10 Diagnosis Expert", ["diagnosis_coding"]),
        ("Procedure Coding Expert", ["procedure_coding"]),
        ("Evidence Extraction Expert", ["fact_extraction"]),
        ("Medical Coding Agent (General)", ["diagnosis_coding", "procedure_coding"]),
    ]:
        if not a2a_registry.get_agent_card(name):
            a2a_registry.register_agent(AgentCard(
    ...
    result = await a2a_registry.coordinate(description, {"query": query}, cap_list)
    return result
...
async def get_a2a_task(task_id: str):
    """Get A2A task status."""
    from app.services.a2a_protocol import a2a_registry
    task = a2a_registry.get_task(task_id)
    if not task:
...
async def cancel_a2a_task(task_id: str):
    """Cancel an A2A task."""
    from app.services.a2a_protocol import a2a_registry
    ok = a2a_registry.update_task(task_id, "cancelled")
    if not ok:
        raise HTTPException(status_code=404, detail="Task not found")
    return {"status": "cancelled", "task_id": task_id}


@router.post("/a2a/tasks/{task_id}/retry")
async def retry_a2a_task(task_id: str):
    ...
    a2a_registry.update_task(task_id, "pending")
    return {"status": "retrying", "task_id": task_id, "previous_status": task.status}


@router.post("/a2a/chain")
async def create_agent_chain(
    input_query: str = Query(...),
    agent_sequence: str = Query(...),  # comma-separated: "EvidenceExtractionExpert,ICDDiagnosisExpert"
    db: AsyncSession = Depends(get_db),
):
    """Execute agents in serial chain with real Expert execution."""
    from app.services.a2a_protocol import a2a_registry, AgentCard
    from app.services.expert_runner import expert_runner

    # Build execute callback that calls real Experts
    async def execute_expert(agent_name: str, input_text: str) -> str:
        result = await db.execute(
            select(Expert).where(Expert.name.ilike(f"%{agent_name}%"))
        )
        exp = result.scalar_one_or_none()
        if exp:
            return await expert_runner.run(exp, input_text, [], [])
        return f"Expert {agent_name} not found in database"

    # Auto-register A2A agents
    for name, caps in [
        ("ICD-10 Diagnosis Expert", ["diagnosis_coding"]),
        ("Procedure Coding Expert", ["procedure_coding"]),
        ("Evidence Extraction Expert", ["fact_extraction"]),
    ...
    seq = [s.strip() for s in agent_sequence.split(",")]
    result = await a2a_registry.chain({"query": input_query}, seq, execute_fn=execute_expert)
    return result
...
async def list_a2a_tasks():
    """List all A2A tasks."""
    from app.services.a2a_protocol import a2a_registry
    tasks = a2a_registry.get_all_tasks()
