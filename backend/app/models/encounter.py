...
class Encounter(Base, TimestampMixin):
    ...
    admission_reason: Mapped[Optional[str]] = mapped_column(EncryptedPHIText(), nullable=True)  # 主诉
    discharge_summary: Mapped[Optional[str]] = mapped_column(EncryptedPHIText(), nullable=True)
    existing_diagnosis_codes: Mapped[dict] = mapped_column(EncryptedPHIJSON(), default=list)  # [{code, name}]
    existing_procedure_codes: Mapped[dict] = mapped_column(EncryptedPHIJSON(), default=list)
...
class Document(Base, TimestampMixin):
    ...
    content: Mapped[str] = mapped_column(EncryptedPHIText(), nullable=False)
