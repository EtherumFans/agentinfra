"""Governed local Provider for source-bound clinical education."""
...
class GovernedClinicalEducationProvider:
    provider_id = "icoder.governed-clinical-education.v1"
    backend_type: BackendType = "rule_engine"
    supports_tool_calling = False
    supports_streaming = False
    deterministic = True
    _OUTPUT_CONTRACT_REF = "icoder/ClinicalEducationOutput/v6"
    ...
    async def health(self) -> ProviderHealth:
        try:
            ...
            return ProviderHealth(
                ...
                details={
                    "provider_id": self.provider_id,
                    "error": f"clinical_education_health_failed:{type(exc).__name__}",
                },
            )
    ...
    async def invoke(
        ...
        *,
        ...
        try:
            ...
            internal = build_clinical_education(text, run_id=ctx.run_id)
            public = to_pack_output(internal)
            input_required = internal["education_status"] == "INPUT_REQUIRED"
            ...
            return BackendResponse(
                status=status,
                summary=(
                    "缺少明确主题、受众或批准来源原文；未生成教学内容。"
                    if input_required else (
                        f"已绑定 {internal['_trace']['evidence_items_count']} 条来源证据；"
                        "未调用外部知识或生成临床建议，必须人工复核。"
                    )
                ),
                ...
                finish_reason=(
                    "required_labelled_clinical_education_fields_missing"
                ...
                evidence_refs=[],
                trace_refs=[f"{ctx.run_id}:governed-clinical-education"],
            )
        except Exception as exc:
            latency_ms = int((time.perf_counter() - started) * 1000)
            logger.error(
                "GovernedClinicalEducationProvider.invoke failed error_type=%s",
                type(exc).__name__,
            )
            self._emit_backend_metadata(ctx, {}, latency_ms, "fail")
            return BackendResponse(
                status="fail",
                summary="本地来源绑定临床教育编排无法安全执行。",
                latency_ms=latency_ms,
                cost_usd=0.0,
                finish_state="failed",
                finish_reason=f"governed_clinical_education_error:{type(exc).__name__}",
                backend_provider=self.provider_id,
                backend_type=self.backend_type,
            )
    ...
    def capabilities(self) -> ProviderCapability:
        return ProviderCapability(
            ...
            supported_tools=[],
            description=(
                "Copies explicitly labelled metadata and statements from an "
                "approved hospital source into fixed learning-objective, key-point "
                "and open-response knowledge-check templates with exact spans. It "
                "does not retrieve literature, classify clinical questions, reason "
                "clinically, assess drugs, calculate scores, advise patients, or write back."
            ),
        )
    ...
    def _emit_backend_metadata(
        ...
        try:
            ...
            logger.warning(
                "GovernedClinicalEducationProvider trace emit failed error_type=%s",
                type(exc).__name__,
            )


__all__ = ["GovernedClinicalEducationProvider"]
