"""Deterministic ICD-10-CN index navigation over governed local assets."""
...
class GovernedICDNavigatorProvider:
    """Surface catalog candidates and one-level hierarchy without an LLM."""

    provider_id = "icoder.governed-icd-navigator.v1"
    backend_type: BackendType = "rule_engine"
    supports_tool_calling = False
    supports_streaming = False
    deterministic = True
    _OUTPUT_CONTRACT_REF = "icoder/Icd10NavigatorOutput/v4"

    async def health(self) -> ProviderHealth:
        try:
            from official_agents.icd10_navigator.agent import (
                verify_navigator_health,
            )

            evidence = verify_navigator_health()
            ...
            return ProviderHealth(
                ...
                details={
                    "provider_id": self.provider_id,
                    "error": f"navigator_health_failed:{type(exc).__name__}",
                },
            )
    ...
    async def invoke(
        ...
        *,
        ...
        try:
            ...
            search_status = str(public.get("search_status") or "")
            unavailable = search_status == "CATALOG_UNAVAILABLE"
            input_required = search_status == "INPUT_REQUIRED"
            ...
            return BackendResponse(
                status=status,
                summary=str(public.get("summary") or "ICD-10-CN 索引导航已完成。"),
                ...
                finish_reason=(
                    ...
                    if unavailable
                    else "explicit_index_term_required"
                    if input_required
                    else None
                ),
                ...
                trace_refs=[f"{ctx.run_id}:governed-icd-navigation"],
            )
        except Exception as exc:
            latency_ms = int((time.perf_counter() - started) * 1000)
            logger.error(
                "GovernedICDNavigatorProvider.invoke failed error_type=%s",
                type(exc).__name__,
            )
            self._emit_backend_metadata(ctx, {}, latency_ms, "fail")
            return BackendResponse(
                status="fail",
                summary="治理目录索引导航无法安全执行。",
                latency_ms=latency_ms,
                cost_usd=0.0,
                finish_state="failed",
                finish_reason=f"governed_icd_navigation_error:{type(exc).__name__}",
                backend_provider=self.provider_id,
                backend_type=self.backend_type,
            )
    ...
    def capabilities(self) -> ProviderCapability:
        return ProviderCapability(
            ...
            supported_tools=[],
            description=(
                "Hash-pinned ICD-10-CN term lookup and one-level hierarchy "
                "navigation. Candidates remain unverified and human-review-only."
            ),
        )
    ...
    def _emit_backend_metadata(
        ...
        try:
            ...
            emit_backend_metadata_event(
                ...
                candidate_codes_count=int(
                    trace_refs.get("candidate_codes_count") or 0
                ),
                query_terms_count=int(trace_refs.get("query_terms_count") or 0),
                rephrasing_attempted=bool(
                    trace_refs.get("rephrasing_attempted")
                ),
                store=get_default_store(),
            )
        except Exception as exc:
            logger.warning(
                "GovernedICDNavigatorProvider trace emit failed error_type=%s",
                type(exc).__name__,
            )


__all__ = ["GovernedICDNavigatorProvider"]
