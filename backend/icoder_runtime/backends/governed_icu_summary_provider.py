"""Governed local Provider for explicitly documented ICU admission facts."""
...
class GovernedIcuSummaryProvider:
    provider_id = "icoder.governed-icu-summary.v1"
    backend_type: BackendType = "rule_engine"
    supports_tool_calling = False
    supports_streaming = False
    deterministic = True
    _OUTPUT_CONTRACT_REF = "icoder/IcuSummaryOutput/v3"

    async def health(self) -> ProviderHealth:
        try:
            from official_agents.icu_summary.agent import verify_icu_summary_health

            evidence = verify_icu_summary_health()
            ...
            return ProviderHealth(
                ...
                details={
                    "provider_id": self.provider_id,
                    "error": f"icu_summary_health_failed:{type(exc).__name__}",
                },
            )
    ...
    async def invoke(
        ...
        *,
        ...
        try:
            from official_agents.icu_summary.agent import (
                build_icu_summary,
                to_pack_output,
            )
            ...
            internal = build_icu_summary(text, run_id=ctx.run_id)
            public = to_pack_output(internal)
            input_required = internal.get("summary_status") == "INPUT_REQUIRED"
            ...
            return BackendResponse(
                status=status,
                summary=(
                    "请按明确 ICU 入院字段提供可核验记录。"
                    if input_required
                    else (
                        f"已整理 {internal['_trace']['evidence_items_count']} 条 ICU 入院原文证据；"
                        "未计算评分、筛查药物或生成治疗建议，须 ICU 医师复核。"
                    )
                ),
                latency_ms=latency_ms,
                cost_usd=0.0,
                finish_state="input-required" if input_required else "completed",
                finish_reason="labelled_icu_admission_fields_required" if input_required else None,
                ...
                evidence_refs=[],
                trace_refs=[f"{ctx.run_id}:governed-icu-summary"],
            )
        except Exception as exc:
            latency_ms = int((time.perf_counter() - started) * 1000)
            logger.error(
                "GovernedIcuSummaryProvider.invoke failed error_type=%s",
                type(exc).__name__,
            )
            self._emit_backend_metadata(ctx, {}, latency_ms, "fail")
            return BackendResponse(
                status="fail",
                summary="本地 ICU 入院摘要无法安全执行。",
                latency_ms=latency_ms,
                cost_usd=0.0,
                finish_state="failed",
                finish_reason=f"governed_icu_summary_error:{type(exc).__name__}",
                backend_provider=self.provider_id,
                backend_type=self.backend_type,
            )
    ...
    def capabilities(self) -> ProviderCapability:
        return ProviderCapability(
            ...
            supported_tools=[],
            description=(
                "Reorganizes explicitly labelled ICU admission facts with exact spans. "
                "It does not score acuity, apply reference thresholds, screen medication, "
                "infer clinical context, recommend treatment or write to the EHR."
            ),
        )
    ...
    def _emit_backend_metadata(
        ...
        try:
            ...
            logger.warning(
                "GovernedIcuSummaryProvider trace emit failed error_type=%s",
                type(exc).__name__,
            )


__all__ = ["GovernedIcuSummaryProvider"]
