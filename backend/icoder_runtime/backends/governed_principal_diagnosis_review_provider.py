"""Governed local Provider for documented principal-diagnosis draft review."""
...
class GovernedPrincipalDiagnosisReviewProvider:
    provider_id = "icoder.governed-principal-diagnosis-review.v1"
    backend_type: BackendType = "rule_engine"
    supports_tool_calling = False
    supports_streaming = False
    deterministic = True
    _OUTPUT_CONTRACT_REF = "icoder/PrincipalDxReview/v11"
    ...
    async def health(self) -> ProviderHealth:
        try:
            ...
            return ProviderHealth(
                ...
                details={
                    "provider_id": self.provider_id,
                    "error": f"principal_diagnosis_review_health_failed:{type(exc).__name__}",
                },
            )
    ...
    async def invoke(
        ...
        *,
        ...
        try:
            ...
            internal = build_principal_diagnosis_review(text, run_id=ctx.run_id)
            ...
            return BackendResponse(
                status=status,
                summary=(
                    "缺少明确的编码员初稿、候选集合、版本或证据；未形成主诊断复核包。"
                    if input_required else (
                        f"已核对 {internal['_trace']['candidate_count']} 个明确候选及 "
                        f"{internal['_trace']['selection_basis_count']} 条声明依据；"
                        "未选择、分配或替换主诊断，必须由编码员复核。"
                    )
                ),
                ...
                evidence_refs=[],
                trace_refs=[f"{ctx.run_id}:governed-principal-diagnosis-review"],
            )
        except Exception as exc:
            latency_ms = int((time.perf_counter() - started) * 1000)
            logger.error(
                "GovernedPrincipalDiagnosisReviewProvider.invoke failed error_type=%s",
                type(exc).__name__,
            )
            self._emit_backend_metadata(ctx, {}, latency_ms, "fail")
            return BackendResponse(
                status="fail",
                summary="本地主诊断初稿证据复核无法安全执行。",
                latency_ms=latency_ms,
                cost_usd=0.0,
                finish_state="failed",
                finish_reason=f"governed_principal_diagnosis_review_error:{type(exc).__name__}",
                backend_provider=self.provider_id,
                backend_type=self.backend_type,
            )
    ...
    def capabilities(self) -> ProviderCapability:
        return ProviderCapability(
            ...
            supported_tools=[],
            description=(
                "Checks only a coder-documented principal-diagnosis draft against "
                "an explicit candidate set and exact source spans. It does not extract, "
                "assign, rank, recommend, replace, clinically validate, submit, or write back."
            ),
        )
    ...
    def _emit_backend_metadata(
        ...
        try:
            ...
            emit_backend_metadata_event(
                ...
                candidate_codes_count=int(trace.get("candidate_count") or 0),
                store=get_default_store(),
            )
        except Exception as exc:
            logger.warning(
                "GovernedPrincipalDiagnosisReviewProvider trace emit failed error_type=%s",
                type(exc).__name__,
            )


__all__ = ["GovernedPrincipalDiagnosisReviewProvider"]
