"""Governed local Provider for documented prior-authorization evidence."""
...
class GovernedPriorAuthorizationProvider:
    provider_id = "icoder.governed-prior-authorization.v1"
    backend_type: BackendType = "rule_engine"
    supports_tool_calling = False
    supports_streaming = False
    deterministic = True
    _OUTPUT_CONTRACT_REF = "icoder/PriorAuthorizationOutput/v5"

    async def health(self) -> ProviderHealth:
        try:
            from official_agents.prior_auth.agent import (
            ...
            return ProviderHealth(
                ...
                details={
                    "provider_id": self.provider_id,
                    "error": f"prior_authorization_health_failed:{type(exc).__name__}",
                },
            )
    ...
    async def invoke(
        ...
        *,
        ...
        try:
            ...
            internal = build_prior_authorization(text, run_id=ctx.run_id)
            public = to_pack_output(internal)
            input_required = internal["authorization_status"] == "INPUT_REQUIRED"
            ...
            return BackendResponse(
                ...
                finish_reason=(
                    "required_labelled_prior_authorization_fields_missing"
                    if input_required
                    else None
                ),
                ...
                evidence_refs=[],
                trace_refs=[f"{ctx.run_id}:governed-prior-authorization"],
            )
        except Exception as exc:
            latency_ms = int((time.perf_counter() - started) * 1000)
            logger.error(
                "GovernedPriorAuthorizationProvider.invoke failed error_type=%s",
                type(exc).__name__,
            )
            self._emit_backend_metadata(ctx, {}, latency_ms, "fail")
            return BackendResponse(
                status="fail",
                summary="本地预授权证据装配无法安全执行。",
                latency_ms=latency_ms,
                cost_usd=0.0,
                finish_state="failed",
                finish_reason=f"governed_prior_authorization_error:{type(exc).__name__}",
                backend_provider=self.provider_id,
                backend_type=self.backend_type,
            )
    ...
    def capabilities(self) -> ProviderCapability:
        return ProviderCapability(
            ...
            supported_tools=[],
            description=(
                "Assembles explicitly headed prior-authorization facts and policy "
                "metadata into a fixed review-only packet with exact spans. It "
                "does not assess medical necessity, retrieve payer policy, use "
                "clinical calculators, validate codes, submit, or write back."
            ),
        )
    ...
    def _emit_backend_metadata(
        ...
        try:
            ...
            logger.warning(
                "GovernedPriorAuthorizationProvider trace emit failed error_type=%s",
                type(exc).__name__,
            )


__all__ = ["GovernedPriorAuthorizationProvider"]
