"""Build a scoped live-model evidence bundle for external-LLM Agents.

The local 24-Agent bundle deliberately excludes external models.  This module
validates the complementary authoritative subset and requires a real model
call in every happy, adversarial, and stability RunTrace.  The resulting
bundle is still synthetic development evidence, not clinical validation.
"""
...
from scripts.corti_parity.build_agent_hub_runtime_matrix import build_matrix  # noqa: E402
...
EXPECTED_EXTERNAL_AGENT_COUNT = 2
EXTERNAL_QUALITY_SCOPE = (
    "fresh_external_model_http_semantic_safety_stability_not_clinical_accuracy"
)
DEFAULT_OUT_DIR = REPO_ROOT / "reports" / "agent_hub" / "external_semantic_evidence"


def _external_agent_ids(
    agents_dir: Path,
    *,
    ...
    agent_ids = sorted(
        ...
        and row.get("external_llm_dependent") is True
    )
    if len(agent_ids) != EXPECTED_EXTERNAL_AGENT_COUNT:
        raise ValueError(
            "authoritative runtime matrix must expose exactly two external-model Agents"
        )
    return agent_ids
...
def _scoped_snapshot(
    ...
    *,
    ...
    if not set(agent_ids).issubset(full):
        raise ValueError("external Agent scope is not contained in the visible Pack snapshot")
...
def build_external_bundle(
    *,
    ...
    agent_ids = _external_agent_ids(agents_dir, matrix=matrix)
    snapshot = _scoped_snapshot(agents_dir, agent_ids=agent_ids)
    external_agents = set(agent_ids)
    ...
    for item in agent_results:
        item["external_semantic_live_e2e_verified"] = bool(
            not errors and all(item["gates"].values())
        )

    bundle: dict[str, Any] = {
        "schema_version": "icoder.agent-hub-external-semantic-evidence-bundle/v1",
        "generated_at": current.isoformat(),
        "quality_scope": EXTERNAL_QUALITY_SCOPE,
        "max_age_hours": max_age_hours,
        "valid": not errors and expected_count == EXPECTED_EXTERNAL_AGENT_COUNT,
        "errors": errors,
        "summary": {
            "visible_agents": len(_snapshot_from_agents_dir(agents_dir)),
            "external_agents_expected": expected_count,
            "external_semantic_live_e2e_verified": sum(
                bool(item["external_semantic_live_e2e_verified"])
                for item in agent_results
            ),
            "local_agents_not_evaluated": (
                len(_snapshot_from_agents_dir(agents_dir)) - expected_count
            ),
        },
        "sources": {
            label: {
        ...
        "limitations": [
            "This proves two external-model-dependent Agents over fresh loopback HTTP with real model-call telemetry; it does not cover the other twenty-four Agents.",
            "Pack-owned synthetic cases are not independent clinical gold, Corti parity, hospital acceptance, or production approval.",
            "Credentials must remain process-scoped and must not be written to reports, logs, command lines, or source files.",
        ],
    }
...
def validate_external_bundle_file(
    bundle_path: Path,
    *,
    agents_dir: Path = DEFAULT_AGENTS_DIR,
    max_age_hours: float = 24.0,
    ...
    if supplied.get("schema_version") != (
        "icoder.agent-hub-external-semantic-evidence-bundle/v1"
    ):
        errors.append("bundle: unsupported schema_version")
    if supplied.get("quality_scope") != EXTERNAL_QUALITY_SCOPE:
        errors.append("bundle: external-model quality scope is missing")
    ...
    if supplied.get("agent_snapshot") != rebuilt.get("agent_snapshot"):
        errors.append("bundle: current external Agent snapshot mismatch")
    if supplied.get("agent_results") != rebuilt.get("agent_results"):
        errors.append("bundle: external Agent verification rows do not match sources")
    if {
    ...
    verified = sorted(
        ...
        if item.get("external_semantic_live_e2e_verified") is True
    )
    if len(verified) != EXPECTED_EXTERNAL_AGENT_COUNT:
        errors.append("bundle: both authoritative external-model Agents must be verified")
    return {
        "valid": not errors,
        "errors": sorted(set(errors)),
...
def write_bundle(out_dir: Path, bundle: dict[str, Any]) -> tuple[Path, Path]:
    ...
    json_path = out_dir / "agent_hub_external_semantic_evidence_bundle.json"
    md_path = out_dir / "agent_hub_external_semantic_evidence_bundle.md"
    ...
    lines = [
        "# Agent Hub external-model semantic evidence bundle",
        "",
        f"Generated: `{bundle['generated_at']}`",
        "",
        f"Validation: **{'PASS' if bundle['valid'] else 'FAIL'}**",
        "",
        f"Fresh external-model HTTP semantic evidence: **{summary['external_semantic_live_e2e_verified']}/{summary['external_agents_expected']}**",
        "",
        "This scoped bundle does not by itself satisfy the full 26-Agent gate.",
        "",
        "## Limitations",
        "",
    ]
