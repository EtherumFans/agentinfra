"""Online KEK rotation by rewrapping PHI data keys at revisions 072 through 076.
...
def run(database_url: str, *, execute: bool, batch_size: int) -> dict:
    ...
    with psycopg.connect(_url(database_url)) as connection:
        with connection.cursor() as cursor:
            ...
            if revision not in {"072", "073", "074", "075", "076"}:
                raise RuntimeError(
                    f"PHI DEK rewrap requires compatible revision 072 through 076; found {revision}"
                )
