...
def _trace_worker(
    ...
    if operation == "append-first":
        event = RunTraceEvent(
            ...
            status="ok",
            ts=1.0,
            safe_metadata={"_organization_id": "org_default1"},
        )
        store.append(event)
        result_queue.put({"event_id": event.event_id})
        return

    before = store.get_run_scoped(run_id, "org_default1")
    event = RunTraceEvent(
        run_id=run_id,
        trace_id=f"trace-{run_id}",
        step="completion",
...
def _run_worker(context, database_url: str, run_id: str, operation: str) -> dict:
    queue = context.Queue()
    process = context.Process(
        target=_trace_worker,
        args=(database_url, run_id, operation, queue),
    )
    process.start()
...
def test_postgres_trace_is_visible_and_ordered_across_processes() -> None:
    ...
    try:
        first = _run_worker(context, database_url, run_id, "append-first")
        second = _run_worker(context, database_url, run_id, "read-then-append")

        reader = DbRunTraceStore()
        events = reader.get_run_scoped(run_id, "org_default1")
