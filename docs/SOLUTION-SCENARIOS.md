...
- Corti 是封闭 SaaS——Expert 黑盒，HIS 厂商无法定制
- iCoDer 是开源 Runtime——Agent 包合同定义 (Hoare-style pre/post conditions)，HIS 厂商可以新增自有工具 (如对接内部药典、医保规则库)
- iCoDer 提供边缘 PHI 脱敏 (在 HIS 厂商部署的 edge node 完成) + 区域数据驻留 (EU/US/CN), 满足《数据安全法》和医院信息科合规要求
...
1. **即开即用**：预置编码审核管道、语音转录、文书生成、事实提取等临床AI能力，医院/ISV 开通 Tenant 后立即使用
2. **PHI 边缘脱敏**：HIS/EMR edge node 完成 PHI 脱敏，原始 PHI 不进入云审计通道，仅脱敏样本入云 (满足《数据安全法》+ 医院信息科合规要求)
...
**iCoDer 适合**：需要中国编码原生支持、需要深度定制、需要确定性医学保障的医院集团和 HIS 厂商。
