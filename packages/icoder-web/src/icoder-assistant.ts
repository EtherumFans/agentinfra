...
class iCoDerAssistant extends HTMLElement {
  ...
  configure(config: { accessToken: string; baseURL: string; refreshToken?: string; mode?: string; language?: string }) {
  ...
  private render() {
    ...
    const input = this.root.getElementById('msg-input') as HTMLInputElement;
    sendBtn?.addEventListener('click', () => this.sendMessage(input));
    input?.addEventListener('keydown', (e) => { if (e.key === 'Enter') this.sendMessage(input); });
  }
