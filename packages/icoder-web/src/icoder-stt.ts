...
class iCoDerStt extends HTMLElement {
  private config: SttConfig = { language: 'zh-CN', interimResults: true, continuous: true };
  ...
  attributeChangedCallback(name: string, _old: string, value: string) {
    if (name === 'language') this.config.language = value;
    this.render();
  }
  ...
  private async start() {
    ...
    if (SR && !this.config.baseURL) {
      ...
      rec.onend = () => { if (this.isListening) rec.start(); };
      try { rec.start(); this.recognition = rec; this.isListening = true; this.render(); }
      catch { this.dispatchEvent(new CustomEvent('error', { detail: { error: '浏览器不支持语音识别' } })); }
      return;
    }
    ...
    try {
      ...
      this.ws.onmessage = (ev) => {
        ...
        if (m.type === 'interim') this.interim = m.text || '';
        else if (m.type === 'final') { this.transcript += m.text || ''; this.interim = ''; this.dispatchEvent(new CustomEvent('transcript', { detail: { text: m.text, isFinal: true } })); }
        this.updateTranscript();
      };
      this.ws.onerror = () => { this.dispatchEvent(new CustomEvent('error', { detail: { error: 'WebSocket连接失败' } })); this.stop(); };
      this.ws.onclose = () => this.stop();
      // Timeout
      setTimeout(() => { if (this.ws && this.ws.readyState !== WebSocket.OPEN) { this.stop(); this.dispatchEvent(new CustomEvent('error', { detail: { error: '连接超时' } })); } }, 5000);
    } catch (err: any) {
