--- BEGIN UNTRUSTED EXTERNAL CONTENT (source: https://docs.corti.ai/.well-known/agent-skills/corti-dictation/SKILL.md) ---
---
...
`"use client"` and **static-import** `@corti/dictation-web` at the top —
don't `next/dynamic` it or `await import()` it. Event payloads sit at
`event.detail.data` (not `event.detail`): `{text, isFinal, ...}` for
`transcript`, `{id, variables, ...}` for `command`.
